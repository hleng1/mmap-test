...
// Function to process a segment of the memory-mapped file
void process_segment(const char *buffer, size_t start, size_t end) {
    for (size_t i = start; i < end; ++i) {
        // Process the buffer[i], e.g., count characters, search for a pattern, etc.
    }
}
...
int main() {
    ...
    for (unsigned int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(process_segment, buffer, start, end);
    }
